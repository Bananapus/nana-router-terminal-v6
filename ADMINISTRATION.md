...
`nana-router-terminal-v6` splits administration between a global registry and project-local terminal selection. The Router and Gateway logic are immutable. The mutable control plane lives in `JBRouterTerminalRegistry`; retry and finalization are permissionless and have no human approval role.
...
- `JBRouterTerminalGateway` has one immutable `ROUTER` and no owner, pause, cancellation, seizure, or discretionary-refund surface
