...
- Cashout-path discovery (`_findCashOutPath` at `:2164-2255`) prioritizes (1) tokens the destination directly accepts via a usable primary terminal, (2) non-JB base tokens that can exit the recursion via a swap, (3) JB project tokens (recursable). Candidate terminals whose ERC-165 or `accountingContextsOf` probes revert are skipped, so one broken source terminal does not DoS enumeration. When no path advances the route, `JBRouterTerminal_NoCashOutPath` reverts — the router never silently drops the user's input on the floor.
