...
The EVM cannot distinguish a permanently broken destination from a route which is temporarily failing or deliberately made to fail. The gateway therefore requires three permissionless attempts, each forwarding at least 5,000,000 gas and separated by at least one day, followed by another one-day wait and a final attempt. All four failures must have the same error selector. Encoded arguments are ignored; a changed selector resets the streak to one and keeps the input retryable. A sink can still alternate selectors to prevent autonomous refund or reproduce one selector through every window, so this is strong evidence rather than a proof of permanent failure.

**Failure classes intentionally ignore encoded error arguments.** *(Accepted tradeoff)*
`JBRouterTerminalGateway` hashes only the first four bytes of return data when a selector is present. Empty and shorter failures are hashed as-is. This prevents changing pool quotes, balances, and other live error arguments from resetting qualification while copying at most four bytes of adversarial return data. Different errors with the same selector intentionally share a failure class. An empty failure which consumes the complete forwarded gas budget is not counted because it proves only that the selected budget was inadequate.

**Arbitrarily underfunded outer transactions cannot be made safe by an inner contract.** *(Documented limitation)*
The gateway reserves 750,000 gas around its initial Router attempt so a route-level OOG normally becomes durable custody instead of bubbling into the source terminal's fail-open catch. No EVM callee can guarantee progress when the transaction itself supplies too little gas to reach or persist that fallback. Permissionless source-terminal operations can therefore force a healthy route into custody by constraining the outer transaction, but they cannot advance its failure streak with a gas-exhausted qualified retry. Clients should submit at least 1.5–2x `eth_estimateGas`; keepers can retry with `processPendingCallWithGas` when the 5,000,000-gas default is insufficient. The reported Base transaction and a 750,000–1,500,000 gas sweep are covered by `RouterTerminalGatewayBaseForkTest`.

**Qualified gas budgets are lower-bounded, not capped.** *(Mitigated)*
The default retry and finalization entry points forward 5,000,000 gas. Their `WithGas` variants accept any larger budget, and an empty failure which consumes the complete forwarded budget reverts without advancing qualification. Callers cannot manufacture qualification with a smaller limit, while unusually expensive healthy routes remain recoverable without a new deployment.

**A source terminal can reject its eventual project refund.** *(Documented limitation)*
The gateway never degrades a project-accounting refund into a raw token transfer to a terminal address. It first tries the original source terminal if that terminal remains registered for the source project. If the terminal was removed or rejects the token, the gateway automatically tries `JBDirectory.primaryTerminalOf(sourceProjectId, token)`. This keeps the creditor project and amount immutable while following that project's current accounting configuration. If both terminals reject the refund, finalization reverts atomically and preserves pending custody for another permissionless attempt.
...
**Retention depends on current multi-terminal payer propagation.** *(Pinned integration assumption)*
The Registry propagates a caller's nonzero `IJBPayerTracker.originalPayer()` value. Current `JBMultiTerminal` does not implement that getter, so the Registry records the terminal itself and the Gateway can shape-qualify the failed source-project call. If a future core terminal implements payer tracking, it could propagate the end user instead and disable retention. `test_realCoreMultiTerminalPreservesSourceTerminalRetention` deploys the real dependency and fails if this payer shape changes, forcing an integration review when core is upgraded.
...
**Direct Gateway calls do not use Permit2 metadata.** *(Documented limitation)*
The Registry remains the integration entry point and consumes Permit2 metadata before forwarding tokens into the Gateway. The Gateway itself neither parses Permit2 metadata nor claims `IJBPermitTerminal`, so a caller bypassing the Registry must use an ordinary ERC-20 allowance. Such direct callers also fail synchronously instead of entering project-refund custody unless they satisfy the source-terminal shape and metadata checks.
...
Selecting a gateway as a later Registry default only affects new project cohorts. `Deploy.s.sol` therefore explicitly migrates project 1 and every project ID listed in `NANA_ROUTER_TERMINAL_MIGRATION_PROJECT_IDS`; it reverts if a listed issued project is locked or the deployment caller lacks `SET_ROUTER_TERMINAL`. Operators must include every existing project whose Registry-routed payouts need the Gateway. Locked projects cannot be moved.
