...
**Deployment generations coexist.** A new package or gateway deployment does not migrate existing project pins or default-history cohorts. Resolve the chain's registry `terminalOf(projectId)` before describing a payment as protected by gateway custody. The executed floor-fix rollout selected the gateway for new projects and project 1; the raw router remains unselectable, while existing projects can continue resolving to a retired router until their operators migrate. Canonical deployment records include Ethereum, Optimism, Base, Arbitrum, Sepolia, Base Sepolia, and Arbitrum Sepolia; OP Sepolia remains feed-only with no router or gateway. Keep every deployment generation's ABI for decoding and require a chain's executed deployment record before treating a gateway address as a live route.
...
**Qualified gas budgets escalate for gas exhaustion and never fall back.** *(Mitigated)*
The default retry and finalization entry points target 5M, 10M, 15M, then 20M gas after consecutive empty-data failures, and the minimum never drops below the highest budget already forwarded, so a route whose real failure only appears past the base budget cannot alternate classes forever. `maximumQualifiedCallGas` derives the executable ceiling from the lesser of `block.gaslimit` and the EIP-7825 per-transaction cap (16,777,216 gas) while preserving transaction and failure-accounting reserves, so on chains with large blocks the 20M rung clips to roughly 15M; target steps above that ceiling use the ceiling instead. The `WithGas` variants accept budgets between the current capped step and that ceiling. Callers cannot manufacture qualification with a smaller limit or request an unexecutable larger limit; a healthy route needing more than the base can settle on a later attempt, while a destination which consumes every executable budget cannot lock custody solely by exceeding a fixed cross-chain cap.

**Permissionless retries settle without a slippage floor at a retrier-chosen moment.** *(Accepted tradeoff)*
Retained calls are replayed with `minReturnedTokens = 0` and can never carry a `pay` quote, because the opt-in metadata is the bare 32-byte source-project word. The retrier chooses the block and the pool state. For a token whose only or deepest Uniswap pool the retrier controls, the retained input can be settled at that pool's price; on vanilla V4 routes the Router's bounded-spot fallback allows up to a 15% haircut per attempt. Deep-pool fee tokens (ETH, USDC) bound the capture by the capital needed to out-liquidity the legitimate pool. The alternative, permission-gating retries, trades this for liveness; a keeper-side floor can be added by having retriers use `processPendingCallWithGas` only when the Router's preview meets an off-chain quote.

**A fee payer can manufacture matching failures to have its own fee refunded.** *(Accepted tradeoff)*
Failure classes are matched by selector and the retry route is the Router's liquidity-greedy pool choice, so a payer that can dominate the pool for its fee token (a one-tick position above the legitimate pool's in-range liquidity, or a fresh pool with no observation history) can reproduce one class across three time-separated attempts and finalization, after which the fee is credited back to the source project. Before the Gateway, `JBMultiTerminal` forgave the same fee in the same transaction but added it to `feeFreeSurplusOf`, so a later zero-tax cash out of that balance still paid the fee. The Gateway refund arrives at least three days later without that marker, so a zero-tax cash out of the refunded amount pays no fee. The attack therefore costs three days of time-separated pool domination for a 2.5% fee, whereas before it cost one transaction, but the fee it recovers is fully escaped rather than deferred.

**Payout splits to the fee project are retained like fees.** *(Documented limitation)*
The Gateway cannot distinguish a protocol fee from an ordinary payout split whose recipient is project 1, so such a split is retained if its route fails and, after finalization, returns to the source net of the fee `JBMultiTerminal` already charged, instead of core's gross fee-free restore. Requires the fee project's route to stay broken across the full retry window.

**Retained memos are capped.** *(Mitigated)*
Retriers resupply the memo as calldata, so a memo of tens of kilobytes would push the top retry rung past what a single transaction can carry under the EIP-7825 cap. Calls with a memo longer than 4,096 bytes are never retained and fail synchronously; core payers send empty memos.

**The budget floor can be raised to the ceiling by the first retrier.** *(Accepted tradeoff)*
Because the qualified minimum never drops below the highest budget already forwarded, a griefer can call `processPendingCallWithGas` with the executable ceiling on the first, undelayed retry and make every later attempt for that call cost roughly the ceiling in gas. This cannot lock custody; it raises the cost of settling that one call.
...
The core terminal's caught `FeeReverted` path credits both project balance and `feeFreeSurplusOf`; that counter makes a later zero-tax cash out of the forgiven amount pay the fee it skipped. The gateway can only call the terminal's public `addToBalanceOf` surface, which cannot touch the counter, so an autonomously refunded fee is credited as ordinary project balance and a later zero-tax cash out of it pays no fee. Nonzero-tax cash outs and payouts still charge their normal fee. This is weaker zero-tax fee recovery than core's synchronous path, and it applies only to fees whose route failed the same way across three time-separated qualified attempts plus finalization. It is accepted because the alternative is the pre-Gateway behavior, in which the fee was forgiven in the same transaction on the first failure.

**Raw source-project metadata opts a payment into custody.** *(Accepted tradeoff)*
Metadata which is exactly 32 bytes and encodes a nonzero project ID no wider than `uint64` opts a zero-minimum payment into failed-route custody and fixes that project as the eventual refund creditor. Wider 32-byte words — hashes, packed addresses, and other coincidental payloads — are not opt-ins and keep synchronous failure semantics. This lets immutable protocol payers such as `REVLoans` avoid fee forgiveness without caller authentication or contract changes. A direct caller can forge the metadata, but can escrow only tokens it supplies; after matching failures those tokens are credited to the project it named, not returned to the caller. Calls without the opt-in and all non-zero-minimum payments retain synchronous failure semantics. A source project's own token is never retained, since no terminal of that project can book a refund of it (reserved-token splits keep `JBController`'s beneficiary fallback), and a registered source terminal retains only fees to the fee project (payout splits keep `JBMultiTerminal`'s fee-free restore); both rules key on properties no upstream caller can shape.
