...
- **Leftover token handling.** `_handleSwap` uses a balance-delta pattern to measure swap output. Pre-existing ETH balances in the contract are protected -- only the delta from the current swap operation is refunded. ETH sent directly to the contract (via `receive()`) remains stuck rather than being absorbed into the next swap. There is no sweep mechanism, so accidentally sent ETH is unrecoverable but cannot be stolen by subsequent callers.
...
- **receive() function.** The contract accepts arbitrary ETH via `receive()`. Pre-existing balances are now protected: only the delta from the current swap is refunded to the caller. ETH sent directly to the contract outside of a swap remains stuck (safer than being stolen by the next caller).
