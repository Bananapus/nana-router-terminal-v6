...
The gateway never degrades a project-accounting refund into a raw token transfer to a terminal address. If the predetermined source terminal no longer accepts the token or otherwise reverts, finalization reverts atomically and preserves pending custody. It deliberately does not retarget through `JBDirectory.primaryTerminalOf` at refund time because that would make the creditor mutable after escrow and could redirect custody to a terminal the source call never used. Without a protocol-defined alternate project-accounting destination, autonomous delivery cannot be guaranteed; permissionless finalization can be retried if the source terminal becomes usable again.
