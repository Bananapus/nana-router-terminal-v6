...
Failure classes are matched by selector and the retry route is the Router's liquidity-greedy pool choice, so a payer that can dominate the pool for its fee token (a one-tick position above the legitimate pool's in-range liquidity, or a fresh pool with no observation history) can reproduce one class across three time-separated attempts and finalization, after which the fee is credited back to the source project. This is not a regression: before the Gateway, `JBMultiTerminal` forgave the same fee in the same transaction and marked it fee-free, whereas the Gateway refund arrives at least three days later as ordinary fee-liable balance.
