...
The Gateway implements `IJBPermitTerminal`, parses Permit2 metadata in its own namespace, falls back from direct ERC-20 allowance to Permit2 transfer, and resolves token owners through the same immutable ERC-2771 trusted forwarder used by the Router and Registry. Direct callers still fail synchronously instead of entering project-refund custody unless they satisfy the source-terminal shape and metadata checks.
