...
contract RouterTerminalTest is Test {
    ...
    /// @notice Checks that the buyback swap floor is split into beneficiary and reserved token counts.
    ...
    /// @notice Checks that skipping splits reserves only tokens from the direct-mint leg.
    function test_previewPayFor_honorsBuybackSkipSplitsMetadata() public {
        ...
        // The metadata splits its 1,500-token swap floor at 50% and requests that execution bypass that split.
        // The direct-mint leg still applies the reserve, leaving 50 of its 100 tokens for the beneficiary.
    ...
    /// @notice Checks that an unseeded oracle cannot raise the route score above the swap floor.
    ...
    /// @notice Checks that route selection includes the output described by the canonical buyback hook.
    ...
    /// @notice Checks that a full reserve applies to the entire direct-mint leg while swap tokens bypass splits.
    function test_previewPayFor_reservesWholeDirectMintLegAtFullReserve() public {
        ...
        // The metadata requests that the 1,500-token swap floor bypass splits while all 100 tokens from the
        // direct-mint leg are reserved. Its zero beneficiary share must not hide that direct-mint leg.
        JBPayHookSpecification[] memory hookSpecifications = _buybackSkipSplitsPayHookSpecifications({
    ...
    /// @notice Builds a buyback swap-floor specification with reserved splits.
    ...
    /// @notice Builds a buyback specification with a swap floor and oracle diagnostics.
    /// @param hook The canonical buyback hook supplying the metadata.
    /// @param minimumSwapAmountOut The minimum swap output, in project-token units.
    /// @param reservedPercent The reserved percentage out of 10,000.
    /// @param rawSwapQuote The oracle-derived swap output used when the oracle is seeded.
    /// @param oracleUnseeded Whether the raw swap quote lacks oracle coverage.
    /// @return specifications The single buyback hook specification.
    ...
    /// @notice Builds a mixed buyback specification whose swap output bypasses reserved splits.
    /// @param hook The canonical buyback hook supplying the metadata.
    /// @param minimumSwapAmountOut The minimum swap output, in project-token units.
    /// @param reservedPercent The reserved percentage out of 10,000.
    /// @return specifications The single buyback hook specification with a direct-mint leg.
        ...
        returns (JBPayHookSpecification[] memory specifications)
    {
        ...
        specifications[0] = JBPayHookSpecification({
            hook: IJBPayHook(hook),
            noop: false,
            // 100 input units are swapped and 50 input units mint directly using an 18-decimal weight ratio.
            amount: 100,
            metadata: abi.encode(
                false,
                uint256(50),
